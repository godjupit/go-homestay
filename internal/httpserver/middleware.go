...
func JWT(secret string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil || !token.Valid {
			c.AbortWithStatusJSON(http.StatusUnauthorized, errorResponse{Code: shared.CodeToken, Msg: "token失效，请重新登陆"})
			return
		}
...
func RequirePermission(adminSvc *admin.Service, code string) gin.HandlerFunc {
	return func(c *gin.Context) {
		c.Set("permissionCode", code)
		auth, err := adminSvc.Authorization(c.Request.Context(), AdminID(c))
		if err != nil {
			Fail(c, err)
			c.Abort()
			return
		...
		if _, ok := auth.Permissions[code]; !ok {
			...
			c.AbortWithStatusJSON(http.StatusForbidden, errorResponse{Code: shared.CodeForbidden, Msg: "无此操作权限"})
			return
		}
		c.Next()
	}
...
func AdminAudit(adminSvc *admin.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		start := time.Now()
		requestID := c.GetHeader("X-Request-ID")
		if requestID == "" {
			requestID = shared.Random(16)
		}
		...
		audit := &admin.AdminAudit{AdminUserID: AdminID(c), Username: fmtString(username), PermissionCode: fmtString(permission), Method: c.Request.Method, Path: c.Request.URL.Path, RequestID: requestID, IP: c.ClientIP(), HTTPStatus: c.Writer.Status(), Success: c.Writer.Status() < 400 && errorMessage == nil, DurationMS: time.Since(start).Milliseconds(), RequestBody: body, ErrorMessage: fmtString(errorMessage)}
		go func() {
			ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
			defer cancel()
			if err := adminSvc.SaveAudit(ctx, audit); err != nil {
